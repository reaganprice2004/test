...
using namespace std; // standard namespace (cin, cout)

// function takes pointer to integer array and its size, return sum of all elements in array
int sumArray(int* arr, int size) {
    int sum = 0; // initialize sum to 0
    for (int i = 0; i < size; i++) { // loop through each element using for
...
int main() {
    ...
    cout << "--- Pointer Display ---" << endl; // display header
    ...
    cin >> size; // read size given by user

    int* arr = new int[size]; // allocate array of ints

    cout << "Enter " << size << " integer values (with a space/newline in between each number):" << endl; // prompt user to fill array
    ...
    int total = sumArray(arr, size); // call sumArray function (from above) to get sum of elements
    cout << "Sum of array elements: " << total << endl; // display sum

    delete[] arr; // free dynamically allocated memory (deallocate memory when using new())
